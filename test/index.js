...
describe('1. web server', () => {
  ...
  it('1.1. listening port 1234', () => {
    const { port } = server.address()
    assert.strictEqual(port, getPort())
  })

  it('1.2. get html from expected route', async (t) => {
    let html
  ...
  it('1.2 post html from unexpected route should return 405', async (t) => {
    try {
      const res = await fetch(`http://localhost:${getPort()}/`, {
        method: 'POST'
      })
  ...
  it('1.3. get image from expected route', async (t) => {
    let image
  ...
  it('1.4. get 404 from unexpected route', async (t) => {
    let html
  ...
  it('1.5 handle a POST request', async (t) => {
    const data = {
      name: 'midudev',
  ...
  it('1.5 avoid handle something other than POST request', async (t) => {
    const data = {
      name: 'midudev',
      email: 'midudev@gmail.com',
      message: 'Hola mundo'
